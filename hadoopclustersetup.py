...
y = commands.getoutput('sudo docker run -it -d -h nn.example.com sound' )
containerId.append(y)

##creating docker container for jobtracker
y = commands.getoutput('sudo docker run -it -d -h jobt.example.com sound' )
containerId.append(y)

##creating docker container for datanodes
count=1
for i in range(n-2)  :
    y= commands.getoutput("sudo docker run -it -d -h dn"+str(count)+".example.com sound")
    containerId.append(y)
    count+=1
